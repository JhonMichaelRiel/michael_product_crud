...
This project now includes an authenticated products CRUD application:

- `GET /login` and `POST /login` provide session-based authentication.
- `/products`, `/products/create`, `/products/edit/{id}`, and `/products/delete/{id}` are protected.
- Products support create, read, update, and delete operations.
- Database schema is defined in `app/migrations/003_create_products_table.php`.
- Migration `004_seed_admin_user.php` creates the admin account from environment variables.

### Local setup

1. Copy `.env.example` to `.env` and fill in the MySQL credentials and admin account values.
2. Set `MIGRATION_ENABLED=true` temporarily in `.env`.
3. Apply the numbered files in `app/migrations/` using your normal LavaLust migration runner, or execute their equivalent SQL in Aiven MySQL. The bundled CLI currently does not expose a `migrate` command.
4. Set `MIGRATION_ENABLED=false` after the schema is created.
5. Open `http://localhost/michael_crud_product/login`, or run the built-in server:

```bash
php -S 127.0.0.1:3000 -t public public/index.php
```

### Aiven and Render variables

Set these as secrets/environment variables. Do not commit `.env` or database passwords:
...
Run the migration once on Render, then change `MIGRATION_ENABLED` to `false` and redeploy. The Render PHP start command should be `php -S 0.0.0.0:$PORT -t public public/index.php`.

### Verification checklist

- Visiting `/products` without a session redirects to `/login`.
- A seeded user can log in and access the product list.
- Add, edit, and delete use POST forms with CSRF protection.
- Product rows are persisted in the Aiven `products` table.
- Capture the login, list, add, edit, delete, database table, GitHub URL, and Render URL for submission.
